...
| Live conversation history (transcript + LLM turns for an in-progress session only) | Supabase Postgres (`agent_conversation_turns` table), `apps/web` backend | **Decided 2026-07-24 (T-002):** a Supabase table, not a short-TTL cache — one default backend per product profile (`.claude/rules/architecture.md`), no Redis/Upstash/Vercel-KV account exists or is planned, and the cache's main advantage (TTL expiry) is approximated with an `expires_at` column + a defense-in-depth cleanup function. See `docs/30-engineering/DATA_MODEL.md`'s decision record and `packages/database/supabase/migrations/20260724120200_agent_conversation_turns.sql` for full rationale. Held only for the duration of the live session; deleted explicitly at `POST /api/agent/session/end` (natural end or cap-triggered) — the `expires_at`/cleanup mechanism is a backstop only, for the case where that call never fires. Zero retention beyond the live session by default (ADR-001 D7, amended) — this is a deliberate tightening versus the originally-planned 30-day window, since there is no vendor platform managing retention anymore |
...
- ~~The choice of storage mechanism for live conversation history (Supabase table vs. a short-TTL cache, ADR-001 D7 amended) is not yet decided~~ — **resolved 2026-07-24 (T-002):** a Supabase table (`agent_conversation_turns`), not a separate cache. See the Data ownership table above, `docs/30-engineering/DATA_MODEL.md`'s decision record, and the rationale comment in `packages/database/supabase/migrations/20260724120200_agent_conversation_turns.sql`. The primary deletion guarantee is still the explicit `DELETE` at `POST /api/agent/session/end` (T-035, not yet implemented); the table adds an `expires_at` defense-in-depth backstop only, not yet activated (no live project to schedule a cleanup job against).
